	  ...
	  period(display_period)
{
				...
				0, buffer, width, height, 8, 0);
	{
		...
		for (int i = 0; i < imgsize; i++) buffer[i] = 0;
	}
	image->data = buffer;
...
LCDC::read(const ensitlm::addr_t &a, ensitlm::data_t &d)
{
	switch (a)
	{
	case LCDC_ADDR_REG:
...
// Write transactions
tlm::tlm_response_status
LCDC::write(const ensitlm::addr_t &a, const ensitlm::data_t &d)
{
	switch (a)
	{
	case LCDC_ADDR_REG:
...
// Reads an external memory devices and fill the buffer corresponding to the internal XImage
